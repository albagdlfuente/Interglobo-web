...
class bp_course_notifications{
	...
	*/
	...
	function student_message_manage_user_application($action,$course_id,$user_id){
	    ...
	    if(bp_is_active('messages') && function_exists('bp_course_messages_new_message'))
	      bp_course_messages_new_message( array('sender_id' => $sender_id, 'subject' => sprintf(__('Application %s by Instructor','vibe'),$action), 'content' => $message,   'recipients' => $user_id ) );
	}
	...
	function instructor_message_manage_user_application($action,$course_id,$user_id){
		...
		$message = sprintf(__('Application %s for course %s ','vibe'),$action,' <a href="'.$this->get_permalink($course_id).'">'.$this->get_the_title($course_id).'</a>');
	    if(bp_is_active('messages') && function_exists('bp_course_messages_new_message'))
	      bp_course_messages_new_message( array('sender_id' => $sender_id, 'subject' => sprintf(__('Application %s by Instructor','vibe'),$action), 'content' => $message,   'recipients' => $receiver_id ) );
	}
