    ...
    */
    	...
    	*/
    	...
    	if(isset($loggedin_menu) && $loggedin_menu!= ''){
    		...
    		if(!count($loggedin_menu)){return array();}else{
			...
			foreach($loggedin_menu as $component  => $item){
				...
				if(isset($item['label']) && strpos($item['label'],'{{notification_count}}') !== false){
					$n=vbp_current_user_notification_count();
					$loggedin_menu[$component]['label']=preg_replace("/{{notification_count}}/",'<span>'.$n.'</span>', $item['label']);
				}
				if(isset($item['label']) && strpos($item['label'],'{{inbox_count}}') !== false){
					$loggedin_menu[$component]['label']=preg_replace("/{{inbox_count}}/",(messages_get_unread_count()?' <span>' . messages_get_unread_count() . '</span>':''), $item['label']);
				}
	...
	function check_active_components($loggedin_menu){
		...
		if (!function_exists('WC') && isset($item['orders'])) {
			unset($loggedin_menu['orders']);
		}

		if (!function_exists('pmpro_getMembershipLevelsForUser') && isset($item['orders'])) {
